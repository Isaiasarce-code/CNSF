...
if uploaded_file:
    ...
    if all(col in df.columns for col in columnas_requeridas):
        ...
        entidad_opcion = st.sidebar.selectbox("Selecciona Entidad", ["(Todos)"] + sorted(df["Entidad"].dropna().unique().tolist()))
        modalidad_opcion = st.sidebar.selectbox("Selecciona Modalidad", ["(Todos)"] + sorted(df["Modalidad"].dropna().unique().tolist()))
        ciclo_opcion = st.sidebar.selectbox("Selecciona Ciclo", ["(Todos)"] + sorted(df["Ciclo"].dropna().unique().tolist()))
        cultivo_opcion = st.sidebar.selectbox("Selecciona Cultivo", ["(Todos)"] + sorted(df["Cultivo"].dropna().unique().tolist()))
        ...
        if "Tarifa2" in df_filtrado.columns and not df_filtrado["Tarifa2"].empty:
            resumen_general = pd.DataFrame({
                ...
                "Valor": [df_filtrado["Tarifa2"].mean(), df_filtrado["Tarifa2"].std(), df_filtrado["Tarifa2"].max()]
            })
            ...
            if "Esquema de aseguramiento" in df_filtrado.columns:
                resumen_por_esquema = (
                    ...
                    .agg(["mean", "std", "max"])
                    .reset_index()
                    .rename(columns={"mean": "Promedio", "std": "Desviación estándar", "max": "Máximo"})
                )
